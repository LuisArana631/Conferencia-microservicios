...
export const mascotaAddNew = (e) => {
    return async ( dispatch ) => {
        ...
        try {
            const resp = await fetchManage( ':3800/mascota', e, 'POST' );
...
export const mascotaLoadItems = () => {
    return async ( dispatch ) => {
        ...
        try{
            const resp = await fetchManage( ':3800/mascota' );
            const body = await resp.json(); 
...
export const mascotaUpdateItem = (e) => {
    return async(dispatch) => {
        try{
            const resp = await fetchManage( ':3800/mascota/'+e.id , e, 'PUT' );
            const body = await resp.json(); 
...
export const mascotaDeleteItem = (e) => {
    return async (dispatch) => {
        try {
            ...
            const resp = await fetchManage( ':3800/mascota/'+e.id , {}, 'DELETE' );
            const body = await resp.json(); 
